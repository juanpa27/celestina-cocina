...
- **Flyer "Menú (estado)"** (`src/components/admin/flyers/MenuBoardFlyer.jsx`): 5ª plantilla en `/admin/flyers`, para postear el menú de una categoría como estado de WhatsApp/IG (referencia visual: grilla de promos estilo Alberdin). Grilla 2 col × hasta 3 filas (hasta 6 platos, priorizando destacados/descuento igual que `CategoryFlyer`), cada plato en tarjeta redondeada con foto + gradient + nombre (Fraunces) + precio (amarillo — acá SÍ va amarillo, es el branding de flyers; el "sin amarillo" era solo para el PDF). Badges "X% OFF"/"Top". Header con eyebrow "Nuestro menú" + nombre de categoría (wrap a 2 líneas si es largo, font 62px) + logo medallón + banda azulejo. Footer CTA WhatsApp con el número de `useConfig`. Usa el mismo selector de categoría que el modo "Categoría".
...
- **Visitas al menú público en el Dashboard, vía Vercel Web Analytics** (a pedido: ver las visitas sin entrar al dashboard de Vercel). `@vercel/analytics` ya estaba instalado y montado (sesión 2026-06-30) — lo que faltaba era leer esos datos de vuelta. Vercel expone una API REST oficial (`api.vercel.com/v1/query/web-analytics/visits/aggregate`, ver `/docs/analytics/web-analytics-api`) que funciona en plan Hobby (reporting window de 1 mes, 50k eventos/mes incluidos).
  ...
  - `src/hooks/useVercelAnalytics.js` (react-query, `staleTime` 5 min) → `src/lib/analyticsStats.js` (`summarizeVisits`: agrega el arreglo diario en hoy/semana/mes) → `src/components/admin/VisitasCard.jsx` (tabs Hoy/Semana/Mes + mini gráfico de barras de los últimos 7 días). Si la función devuelve error (token no configurado todavía, o falla la API de Vercel) la card se oculta sola (`isError` → `return null`) en vez de romper el Dashboard.
  ...
  - **Pendiente de configuración manual (no lo puede hacer Claude)**: en el dashboard de Vercel — (1) confirmar que Web Analytics esté *enabled* para el proyecto (Project Settings → Analytics), (2) crear un Access Token de cuenta (Account Settings → Tokens, scope de solo lectura si el plan lo permite), (3) cargar `VERCEL_TOKEN` y `VERCEL_PROJECT_ID` (+ `VERCEL_TEAM_ID` si aplica) como Environment Variables del proyecto en Vercel, (4) redeploy para que la función los lea. Sin esos tres env vars la card de Visitas queda oculta pero el resto del Dashboard funciona normal.
