...
- **Flyer "Texto + foto"** (`src/components/admin/flyers/TextPhotoFlyer.jsx`): cuarta plantilla en `/admin/flyers`, a pedido explícito con una referencia visual (tagliatelles + tenedor atravesando el texto "TAGLIATELLES" gigante). A diferencia de "Texto Hero" (foto de fondo full-bleed detrás de todo el texto), acá el fondo es azul sólido + patrón geométrico (reusa `DiamondPattern`), el texto gigante va detrás (reusa `autoSplitHeroName`/`calcFontSize`, ambos ahora exportados desde `TextHeroFlyer.jsx`), y la foto va en una tarjeta redondeada **por encima** del texto del medio, con borde amarillo de marca y sombra (sin ninguna zona semitransparente). La foto se sube al toque desde `FlyersPage` (`<input type=file>` → `URL.createObjectURL`, blob local en memoria, se libera con `URL.revokeObjectURL` al cambiar/desmontar) — no se persiste en Supabase Storage ni se asocia a ningún plato del catálogo, es de un solo uso para ese flyer.
  - **Fix**: la primera versión usaba un fundido (`mask-image`, primero lineal top/bottom, después ovalado en las 4 direcciones) para simular un recorte transparente. Se veía mal con fotos reales de fondo blanco/estudio (el caso típico) — la zona semitransparente del borde no funde con el azul oscuro del flyer, deja un halo claro con forma de caja pegado sobre el texto (reportado por el usuario con una foto real de lasaña). Reemplazado por una tarjeta con borde neto (sin fundido): la foto tapa el texto del medio de forma intencional, sin halo, y funciona con cualquier foto sin importar su fondo.
