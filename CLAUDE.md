...
- **Rediseño del flyer "Menú (estado)"** (`MenuStatusFlyer.jsx`), a pedido explícito con specs concretas de la dueña, para que entren más platos con menos necesidad de auto-escalado:
  - **Grilla de 3 columnas** (antes 2): `CARD_W` se recalcula para 3 columnas con `GAP=24`. Foto con `aspect-ratio: 4/3` en vez de alto fijo en px — así la altura de la foto escala con el ancho de la tarjeta (más angosta al pasar a 3 columnas) en vez de quedar desproporcionada. Nombre del plato pasa de Fraunces 28px a **DM Sans 700 16px** (serif no aguanta legible a ese tamaño chico; Fraunces queda reservado para los títulos de categoría). Padding interno de tarjeta 16px→10px. Badges ("X% OFF", "🔥 Top") reducidos a juego con el resto.
  ...
  - **Pie de contacto automático**: antes el flyer no tenía footer (se había sacado en la sesión 2026-07-01) y quien publica el estado de WhatsApp (Ajaka) le agregaba el teléfono a mano, con formato improvisado. Se agregó una franja azul fija al final del flyer con el teléfono del negocio, para que nunca dependa de que alguien lo escriba bien. `formatLocalPhone(phone)` (`lib/utils.js`) convierte el valor guardado en `app_config.whatsapp_negocio` (código de país sin `+`, ej. `595986818441`, mismo formato que usa `CartSidebar` para armar el link de `wa.me`) al formato local con `0` que la gente reconoce (`0986 818 441`) — **a propósito no incluye la URL de Vercel**: es fiel a lo que Ajaka viene agregando en la práctica (solo teléfono), no a un footer de marca más completo tipo `FlyerFooter` de `flyerChrome.jsx`.
