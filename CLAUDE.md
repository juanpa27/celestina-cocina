...
- **Fix: flyer "Texto Hero" no exportaba con Titan One** (`index.html`). El `<link>` de Google Fonts no tenía `crossorigin="anonymous"`, así que `html-to-image` no podía leer las reglas `@font-face` del stylesheet (`SecurityError` al acceder a `cssRules` de un stylesheet cross-origin) y descartaba el embebido del font en la imagen exportada. En Chrome no se notaba (comparte el font cache entre el documento y la imagen exportada), pero en Safari/iOS —el navegador real del admin, dado el trabajo de PWA/splash screens para iPhone— caía a system serif en la descarga. Fix de una línea: agregar `crossorigin="anonymous"` al `<link rel="stylesheet">` de Google Fonts. Verificado con un harness Playwright que llama al `toCanvas` real de `flyer.js`: `SecurityError` desaparece de la consola tras el fix.
